...
// Sample image data with the provided Unsplash image
const images = [
  {
...
export default function PhotosCarousel() {
  ...
  // Preload the active image and adjacent images
  ...
  return (
    <div ref={containerRef} className="relative flex h-screen w-full bg-transparent">
      {/* Thumbnails sidebar - with lazy loading */}
...
function CarouselImages({ images, activeIndex, imagesToPreload }) {
  return (
    <group>
      {images.map(
        (image, index) =>
...
function AnimatedImage({ url, index, activeIndex, total }) {
  ...
  // Add some floating animation to the active image
  useFrame(({ clock }) => {
    ...
    <animated.group position={position} scale={scale} rotation={rotation}>
      <animated.mesh ref={imageRef} opacity={opacity}>
        <Suspense fallback={<ImagePlaceholder width={imageWidth} height={imageHeight} />}>
          <Image url={url} transparent opacity={opacity} scale={[imageWidth, imageHeight, 1]} onLoad={handleLoad} />
        </Suspense>
      </animated.mesh>
    </animated.group>
  )
